 ...
 * @param target   事件监听对象
 */
tz.events.IEvent.prototype.on = function (type,callback,target) {
    var list = null;
 ...
 * @param target 事件监听对象
 */
tz.events.IEvent.prototype.remove = function (type,target) {
    if(this.eventList.hasOwnProperty(type)){
        if(target){
 ...
 * 删除所有
 */
tz.events.IEvent.prototype.removeAll = function () {
    for(var key in this.eventList){
        this.eventList[key].length = 0;
    }
};

tz.events.IEvent.prototype.send = function () {
 ...
 * 全局广播
 */
tz.events.dispatcher = tz.events.IEvent.new();